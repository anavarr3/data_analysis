...
#Can throw away if we can figure out how to add a sum column above
```{r}
col_sum <- transposed_redo_arranged |> 
  mutate(sum_col = rowSums(transposed_redo_arranged[,-1])) |>
...
names(foreclosures_sum_date) <- str_replace(names(foreclosures_2022_date), "x", "")
foreclosures_sum_date
```
...
foreclosure_notices_2022 <- foreclosures_sum_date |> 
  mutate(year = (year(date)),
         full_date=floor_date(date, "month")) |> 
  filter(year == "2022") |> 
  select(!year)
...
foreclosure_notices_2022 |> 
  ggplot() + 
  geom_line(aes(x= full_date, y=sum_col)) +
