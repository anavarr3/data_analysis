...
# Here we tried to format the dates using lubridate instead of the zoo package however we continue to run into the issue of getting a character column instead of a date
transposed_redo_2 <- foreclosures |>
  gather(date, value, -1) |>
  spread(key = zip, value = value) |>
  mutate(date= mdy(date), month_year = paste(month.abb[month(date)], year(date), sep = "_"))
transposed_redo_2
